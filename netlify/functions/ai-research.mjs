...
function cleanResearch(x){
  ...
  return {
    ...
    catalysts:Array.isArray(x.catalysts)?x.catalysts.slice(0,5).map(String):[],
    risks:Array.isArray(x.risks)?x.risks.slice(0,6).map(String):[],
    bearCases:Array.isArray(x.bearCases)?x.bearCases.slice(0,4).map(String):[],
    sources:Array.isArray(x.sources)?x.sources.slice(0,10).map(s=>({
...
export default async (req)=>{
  ...
  try{
    ...
    const msg=e?.name==='AbortError'?`${cfg.provider} AI 深研超过 55 秒，已安全中止`:(e.message||'AI 深研失败');
